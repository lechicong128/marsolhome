        ...
        $dtDriver = Driver::select(
            ...
            ->where(function ($query) use ($lat, $lon, $type_car, $customer_id, $type_payment_mode, $amount, $category_car_detail_id) {
                if (!empty($lat) && !empty($lon)) {
                    $query->where(
                        DB::raw("IF($lat != 0 AND $lon != 0 AND latitude IS NOT NULL AND longitude IS NOT NULL,fnCalcDistanceKM($lat,latitude,$lon,longitude),null)"),
                        '!=',
                        null
                    );
                    $query->where(
                        DB::raw("IF($lat != 0 AND $lon != 0 AND latitude IS NOT NULL AND longitude IS NOT NULL,fnCalcDistanceKM($lat,latitude,$lon,longitude),10000)"),
                        '>=',
                        0
                    );
                    $query->where(
...
function findDriver($transaction_id = 0)
{
    ...
    if (!empty($dtTransaction) && $dtTransaction->status == Config::get('constant')['status_request_driver']) {
        ...
        $countDriver = Driver::select(
            ...
            ->where(function ($query) use ($lat, $lon, $type_car, $customer_id, $type_payment_mode, $amount, $transaction_id, $category_car_detail_id) {
                if (!empty($lat) && !empty($lon)) {
                    $query->where(
                        DB::raw("IF($lat != 0 AND $lon != 0 AND latitude IS NOT NULL AND longitude IS NOT NULL,fnCalcDistanceKM($lat,latitude,$lon,longitude),null)"),
                        '!=',
...
{
    $dtBalance = DB::table('tbl_client_balance_month')->where('customer_id', $customer_id)
        ->where(DB::raw('DATE_FORMAT(CONCAT(year,"-",month,"-",01),"%Y-%m")'), '<', $year . '-' . $month)
        ->sum('balance');
    return $dtBalance;
}
...
function changeBalance($id = 0, $type = '', $type_increase = true, $typeF = '', $customer_id = 0, $balance_default = 0, $date = null)
{
    $dtObject = [];
    if ($type == 'finish') {
        ...
        $dtObject = CustomerClass::select(
            ...
            ->where('tbl_customer_class.id', $id)
            ->first();
    ...
    if (!empty($dtObject)) {
        ...
        if ($type == 'transfer_money') {
            ...
            $clientBalanceReceive = DB::table('tbl_client_balance_month')
                ->where('customer_id', $customer_id_receive)
                ->where('month', $month)
                ->where('year', $year)
                ->first();
            if (!empty($clientBalanceReceive)) {
                $balance = $clientBalanceReceive->balance + $revenue_receive;
                DB::table('tbl_client_balance_month')->where('id', $clientBalanceReceive->id)
                    ->update([
                        'balance' => $balance
                    ]);
            } else {
        ...
        if ($type == 'reward_commission') {
            ...
            $dtRewardCommission = DB::table('tbl_customer_reward_commission_day')
                ->where('customer_id', $customer_id)
                ->where('date', $dateNew)
                ->where('type', $type_commission)
                ->first();
            if (!empty($dtRewardCommission)) {
                $balance_commission_day = $dtRewardCommission->balance + $revenue;
                DB::table('tbl_customer_reward_commission_day')->where('id', $dtRewardCommission->id)
                    ->update([
...
function changeBalanceDriver($id = 0, $type = '')
{
    ...
    if ($type == 'finish') {
        $dtObject = TransactionDriver::select(
            ...
            ->where('check_balance', 0)
            ->where('tbl_transaction_driver.id', $id)
            ->first();
    ...
    if (!empty($dtObject)) {
        ...
        $clientBalance = DB::table('tbl_driver_balance_month')
        ...
        if (!empty($clientBalance)) {
            $balance = $clientBalance->balance + $revenue;
            DB::table('tbl_driver_balance_month')->where('id', $clientBalance->id)
                ->update([
                    'balance' => $balance
                ]);
        } else {
...
function changePoint($id = 0, $type = '', $staff_id = 0, $customer_id = 0, $point = 0, $date = null)
{
    ...
    if ($type == 'transaction') {
        $dtObject = Transaction::select(
            ...
            'tbl_transaction.date as date',
            'tbl_transaction.customer_id as customer_id',
            'tbl_transaction.parent_customer_id as parent_customer_id',
    ...
    } elseif ($type == 'affiliate_product') {
        $dtObject = TransactionItem::select(
            'tbl_transaction_item.id',
            'tbl_transaction.date as date',
            'tbl_transaction_item.customer_id_affiliate as customer_id',
            DB::raw('0 as parent_customer_id'),
            'tbl_transaction_item.total_affiliate as point'
        )
            ->join('tbl_transaction', 'tbl_transaction.id', '=', 'tbl_transaction_item.transaction_id')
            ->where('tbl_transaction.status', 2)
            ->where('tbl_transaction_item.id', $id)
            ->where('tbl_transaction_item.review_id', '!=', 0)
            ->first();
    } elseif ($type == 'transaction_point') {
        $dtObject = Transaction::select(
            'tbl_transaction.id',
            'tbl_transaction.date as date',
            'tbl_transaction.customer_id as customer_id',
            DB::raw('0 as parent_customer_id'),
            'tbl_transaction.point as point'
        )
            ->where('tbl_transaction.id', $id)
            ->first();
    } elseif ($type == 'transaction_point_refund') {
        $dtObject = Transaction::select(
            'tbl_transaction.id',
            'tbl_transaction.date as date',
            'tbl_transaction.customer_id as customer_id',
            ...
            ->where('tbl_transaction.refund_point', '=', 0)
            ->where('tbl_transaction.id', $id)
            ->first();
    } elseif ($type == 'review_items') {
        $fnbAdmin = new \App\Services\AdminService();
        $dtObjects = $fnbAdmin->GetDetailReview($id);
        $dtObject = (object)$dtObjects;
        $customer_id = $dtObject->id_client;
        $dtObject->date = $dtObject->date_review;
    } elseif ($type == 'authenticated_challengeMe') {
        $dtObject = ChallengeMeSubmissions::select(
            'challenge_me_submissions.id',
            'challenge_me_submissions.created_at as date',
            'challenge_me_submissions.created_by as customer_id',
            'challenge_me_submissions.haru_xu as point'
        )
            ->where('challenge_me_submissions.id', $id)
            ->first();
    } elseif ($type == 'plus_refund') {// hoàn điểm khi hoàn thành thử thách
        $dtObject = ChallengeMe::select(
            'tbl_challenge_me.id',
            'tbl_challenge_me.created_at as date',
            'tbl_challenge_me.client_id as customer_id',
            'tbl_challenge_me.deposit as deposit'
        )->where('tbl_challenge_me.id', $id)
            ->first();
    } elseif ($type == 'contribute') {// hoàn điểm khi hoàn thành thử thách
        $dtObject = ChallengeContribute::select(
            'tbl_challenge_contribute.id',
            'tbl_challenge_contribute.created_at as date',
            'tbl_challenge_contribute.client_id as customer_id',
            'tbl_challenge_contribute.money as money',
            'tbl_challenge_contribute.total_haru_xu as total_haru_xu',
            'tbl_challenge_contribute.exchange_point_now as exchange_point_now',
            'tbl_challenge_contribute.id_event_articles as id_event_articles'
        )->where('tbl_challenge_contribute.id', $id)->first();
    }
    if (!empty($dtObject)) {
        $date = _dthuan($dtObject->date);
        ...
        $exchange_point = $fnbAdminService->get_option('exchange_rate_haru_wallet') ?? 1;
        if ($type == 'transaction') {
            $point = $dtObject->point;
            if (empty($customer_id)) {
                $customer_id = $dtObject->parent_customer_id;
            } else {
                $customer_id = $customer_id;
            }
            $customer_id_child = $dtObject->customer_id;
            $dtClientChild = Clients::find($customer_id_child);
            ...
            $type_check = 1;
            $object_type = 'transaction_point_refund';
        } elseif ($type == 'review_items') {
            $point = $fnbAdminService->get_option('number_coins_received_review') ?? 0;
            if (empty($customer_id)) {
                $customer_id = $dtObject->customer_id;
            } else {
                $customer_id = $customer_id;
            ...
            $type_check = 1;
            $object_type = 'authenticated_challengeMe';
        } elseif ($type == 'plus_refund') {
            $point = $dtObject->deposit / $exchange_point;
            if (empty($customer_id)) {
                $customer_id = $dtObject->customer_id;
            } else {
                $customer_id = $customer_id;
            ...
            $type_check = 1;
            $object_type = 'plus_refund';
        } elseif ($type == 'contribute') {
            $exchange_point = $dtObject->exchange_point_now;
            $point = (-1) * $dtObject->total_haru_xu;
            if (empty($customer_id)) {
                $customer_id = $dtObject->customer_id;
            } else {
                $customer_id = $customer_id;
        ...
        if ($type == 'transaction_point_refund') {
            Transaction::where('id', $dtObject->id)->update([
                'refund_point' => 1
            ]);
        }
        ...
        DB::table('tbl_client_point_history')->insert([
            ...
            'point_total' => $point * $exchange_point,
            'type_check' => $type_check,
            'child_id' => $child_id ?? 0,
        ...
        if (!empty($point)) {
            ...
            if ($type == 'authenticated_challengeMe') {
                $request_noti_review = new Request([
                    'type_noti' => 'authenticated_challengeMe',
                ...
                $fnbNoti->addNoti($request_noti_review);
            } else if ($type == 'plus_refund') {
                $request_noti_review = new Request([
                    'type_noti' => 'plus_refund',
                    'arr_object_id' => $arr_object_id,
                    'dtData' => $dtObject,
                    'customer_id' => $customer_id,
                    'point' => $point,
            ...
            if ($type == 'review_items') {
                $request_noti_review = new Request([
                    'type_noti' => 'review_items',
                    'arr_object_id' => $arr_object_id,
                    'dtData' => $dtObject,
                    'customer_id' => $customer_id,
                    'point' => $point,
...
function SendSocketH($data, $channels, $events)
{
    $SockKet = new \App\Services\SocketService();
    $request_socket = new Request([
        'data' => $data,
        'channels' => $channels,
        'event' => $events,
    ]);
    $SockKet->sendSocket($request_socket);
}
...
function total_payment($payment_mode_id = 0, $date_start = null, $date_end = null)
{
    ...
    if (is_numeric($payment_mode_id) && !empty($date_start)) {
        $totalPayment = DB::table('tbl_payment')
        ...
        $totalPaymentDriver = DB::table('tbl_payment_driver')
            ->where('payment_mode_id', $payment_mode_id)
            ->where(function ($query) use ($date_start, $date_end) {
                if (!empty($date_start) && !empty($date_end)) {
                    $query->where('date', '>=', $date_start);
...
function total_payslip($payment_mode_id = 0, $date_start = null, $date_end = null)
{
    ...
    if (is_numeric($payment_mode_id) && !empty($date_start)) {
        $totalPayslip = DB::table('tbl_transfer_money')
            ->where('payment_mode_id', $payment_mode_id)
            ->where(function ($query) use ($date_start, $date_end) {
                if (!empty($date_start) && !empty($date_end)) {
                    $query->where('date', '>=', $date_start);
        ...
        $totalPayslipDriver = DB::table('tbl_pay_slip')
            ->where('payment_mode_id', $payment_mode_id)
            ->where(function ($query) use ($date_start, $date_end) {
                if (!empty($date_start) && !empty($date_end)) {
                    $query->where('date', '>=', $date_start);
...
if (!function_exists('recursiveSetupQaFilter')) {
    function recursiveSetupQaFilter($id = 0, &$output = null, $parent_id = 0, $indent = null)
    {
        $query = DB::table('tbl_setup_qa')->where(function ($query) use ($parent_id, $id) {
            ...
            ->orderByRaw('category_setup_qa_id asc')
            ->get();
        foreach ($query as $key => $item) {
            if ($item->parent_id == $parent_id) {
                $disabled = '';
...
{
    $dem++;
    $items = \App\Models\ClientIntroduce::where('id_client_introduce', $parent_id)->get();
    if ($dem == 25) {
        return $items;
    }
    foreach ($items as $value) {
        array_push($result, $value->id_client);
        get_childs_id_helper($value->id_client, $result, $dem);
    }
}
...
function get_parent_id_referral_level($data, $parentId = 0, $level = 0)
{
    ...
    foreach ($data as $key => $value) {
        if ($value->parent_id == $parentId) {
            ...
            $total_new = isset($value->customer->customer_class) ? $value->customer->transaction->where('status', 1)->sum('total') : 0;
            $total_affiliate = isset($value->customer->customer_class) ? $value->customer->transaction->where('status', 1)->sum('total') : 0;
            ...
            $value->total_old_reward = isset($value->customer->reward_day_profit_sum) ? $value->customer->reward_day_profit_sum->where('type', 1)->where('customer_id', $value->customer_id)->sum('balance') : 0;
...
function get_parent_id_referral_level_html($data, $parentId = 0, &$html = '')
{
    $array = [];
    foreach ($data as $key => $value) {
        if ($value->parent_id == $parentId) {
    ...
    if ($array) {
        ...
        foreach ($array as $item) {
            $image = !empty($item->customer->customer_class->category_card) ? $item->customer->customer_class->category_card->avatar : 'admin/assets/images/avatar.jpg';
            $html .= '<li>';
            $html .= '<a href="admin/clients/view/' . $item->customer->id . '"><img style="width: 50px; border-radius: 50%" src="' . $image . '" /></a>';
            $html .= '<div>' . $item->customer->fullname . '</div>';
            get_parent_id_referral_level_html($data, $item->customer_id, $html);
            $html .= '</li>';
        }
        $html .= '</ul>';
    }
...
function get_parent_id_referral_level_new($data, $parentId = 0, $level = 0, $branch = 0)
{
    ...
    foreach ($data as $key => $value) {
        if ($value->id_client_introduce == $parentId) {
            ...
            $array[] = ($value);
            $children = get_parent_id_referral_level_new($data, $value->id_client, $level + 1, $branch);
...
{
    $items = \App\Models\ClientIntroduce::where('id_client', $id)->first();
    $level = 1;

    if ($items) {
        $parent = $items->id_client_introduce;

        while ($parent > 0) {

            $level++;
            $ktr = \App\Models\ClientIntroduce::where('id_client', $parent)->first();
            if (!$ktr) {
                break;
            }

            $dtLeader = Clients::find($parent);

            if ($dtLeader && $dtLeader->is_leader == 1) {
                // gặp leader gần nhất thì dừng
                break;
            }

            $parent = $ktr->id_client_introduce ?? 0;
        }
    }
...
function send_telegram($message = "", $type = 1)
{
    ...
    $curl = curl_init();
    curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://api.telegram.org/bot' . get_option('token_key_tele') . '/sendMessage?chat_id=' . $chat_id . '&text=' . urlencode($message) . '',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
    ...
    if (!empty($parent_id)) {
        ...
        if (!empty($customer_id_f2)) {
            ...
            if ($dtCustomerF2 >= number_unformat(get_option('condition_f2'))) {
                if (!empty($dtDataCustomer) && $dtDataCustomer->customer_class->total >= number_unformat(get_option('condition_total_package'))) {
...
function send_zalo_admin($phone = 0, $event = null, $data = [])
{
    $baseUrlAdmin = rtrim(config('services.admin.base_url'), '/');
    $response = Http::post("{$baseUrlAdmin}/api/send_zalo", [
        'phone' => $phone,
        'event' => $event,
        'data' => $data,
        'key' => md5(md5(json_encode($data)))
    ]);
    $response = $response->json();
    return $response;
}
function createCodeLeader($customer_id = null)
{
    $characters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
    $charactersLength = strlen($characters);
    $randomString = '';
    ...
    $ktCode = DB::table('tbl_code_leader')->where('code', $randomString)->first();
    if (!empty($ktCode->id)) {
        $randomString = createCodeLeader($customer_id);
    } else {
        DB::table('tbl_code_leader')->insert([
            'code' => $randomString,
            'status' => 1,
            'customer_id' => $customer_id,
...
function createCodeIntroduce()
{
    ...
    $ktCode = DB::table('tbl_clients')->where('code_introduce', $randomString)->first();
    if (!empty($ktCode->id)) {
        $randomString = createCodeIntroduce();
    }
    // return $randomString;
    return '';
}
...
function getListTypeReferral($id = -1, $type = '')
{
    $data = [
        [
            'id' => 1,
            'name' => lang('dt_referral'),
            'color' => '#ffffff',
            'background' => '#5599EC'
        ],
        [
            'id' => 2,
            'name' => lang('dt_referral_review'),
            'color' => '#ffffff',
            'background' => '#FFC559'
        ],
        [
            'id' => 3,
            'name' => lang('dt_referral_order'),
            'color' => '#ffffff',
            'background' => '#20B25D'
        ],
    ];
...
function change_client_address($data = [], $id = 0)
{
    ...
    if (!empty($id)) {
        ...
        if (!is_null($address)) {
            ...
            if (isset($data['default_address'])) {
                $address->default_address = $data['default_address'] ?? 0;
            }
            if (!empty($dataUpdate)) {
                ...
                $address->save();
                if (isset($data['default_address']) && !empty($data['default_address'])) {
                    ClientAddress::where('customer_id', $id_client)
                        ->where('id', '!=', $address->id)
...
//Đánh giá lại user client
function reviewClassClient($id_client = 0)
{
    ...
    $GetReviewCount = $AdminService->getCountReviewClient($request);
    $DataReviewCount = $GetReviewCount->getData(true);
    $reviewCount = $DataReviewCount['data']['count_review'];
    // số lượt giới thiệu thành công
    $affiliateCount = DB::table('tbl_client_introduce')
        ->where('id_client_introduce', $id_client)
        ->where('status', 1)
        ->count();


    $listClass = DB::table('tbl_setting_customer_class')->orderBy('id', 'desc')->get();
    if (empty($listClass)) {
        return 1;
    }
    $listRule = [];
    foreach ($listClass as $key => $value) {
        $list_rules = DB::table('tbl_setting_customer_class_rule')
            ->where('setting_customer_class_id', $value->id)
            ->orderBy('setting_customer_class_id', 'DESC')
            ->get();
        foreach ($list_rules as $k => $v) {
            $listRule[$value->id][$v->type] = $v->rule;
        }
    }

    foreach ($listRule as $setting_customer_class_id => $rule) {
        $rule['affiliate'] = $rule['affiliate'] ?? 0;
        $rule['review'] = $rule['review'] ?? 0;
        if ($reviewCount >= $rule['review'] && $affiliateCount >= $rule['affiliate']) {
            return (int)$setting_customer_class_id;
...
function reviewClassClientApi($id_client = 0)
{
    if (!empty($id_client)) {
        $setting_customer_class_id = reviewClassClient($id_client);
        $customer_class = DB::table('tbl_customer_class')
            ->where('customer_id', $id_client)->first();
        if (!empty($customer_class->id)) {
            if ($customer_class->setting_customer_class_id < $setting_customer_class_id) {
                $SettingCustomerClass = SettingCustomerClass::find($setting_customer_class_id);
                // Cập nhật lại lớp khách hàng
                DB::table('tbl_customer_class')->where('customer_id', $id_client)->update([
                    'setting_customer_class_id' => $setting_customer_class_id,
                    'percent' => $SettingCustomerClass->percent,
                    'updated_at' => date('Y-m-d H:i:s')
                ]);
                DB::table('tbl_history_customer_class')->insert([
                    'customer_id' => $id_client,
                    'setting_customer_class_id' => $customer_class->setting_customer_class_id,
                    'percent' => $customer_class->percent,
                    'created_at' => date('Y-m-d H:i:s'),
                    'updated_at' => date('Y-m-d H:i:s')
                ]);
            }
        ...
        return $setting_customer_class_id;
    }
...
//Đánh giá lại user client
function reviewClassChallenge($id_client = 0)
{
    $Clients = Clients::find($id_client);
    if (empty($Clients)) {
        return 0;
    }
    $rankChallenge = checkRankChallenge($id_client);
    if ($Clients->rank_community != $rankChallenge) {
        $Clients->rank_community = $rankChallenge;
        $Clients->save();

        $dataNotify = [
            'rank_challenge' => $rankChallenge,
            'rank_challenge_old' => $Clients->rank_community,
        ];

        $AdminService = new \App\Services\AdminService();
        $listLang = $AdminService->GetUrlSetings('api/getLanguageCurrent');
        foreach ($listLang['data'] as $key => $value) {
            $InfoRankChallenge = DB::table('tbl_rank_community_translations')->where('id_rank', $rankChallenge)->where('language', $value['code'])->first();
            $dataNotify[$value['code']]['content'] = lang('notify_update_rank_challenge', 'message', [
                'rank_challenge' => $InfoRankChallenge->name
            ], $value['code']);

            $dataNotify[$value['code']]['title'] = lang('title_notify_update_rank_challenge', 'message', [
                'rank_challenge' => $InfoRankChallenge->name
            ], $value['code']);
        }
        notificationCus('update_rank_challenge', 0, $id_client, $dataNotify);
    }
    return $rankChallenge;
}


function checkRankChallenge($id_client = 0)
{
    $countChallengeSuccess = ChallengeMe::where('client_id', $id_client)
        ->where('status', 1)
        ->count();

    $listRank = DB::table('tbl_rank_community')->orderBy('id', 'desc')->get();
    foreach ($listRank as $key => $value) {
        if ($countChallengeSuccess >= $value->challenges_next_rank) {
            return (int)$value->id;
        }
    }
    return 1;
...
function notificationCus($type = 'update_rank_challenge', $staff_id = 0, $customer_id = 0, $dataAppend = [])
{
    $adminNoti = new \App\Services\NotiService();
    $dtObject = [];
    $title_point = '';
    $type_check = 1;
    $object_type = '';
    if ($type == 'update_rank_challenge') {
        $type_check = 1;
        $object_type = 'update_rank_challenge';
    }
    foreach ($dataAppend as $key => $value) {
        $dtObject[$key] = $value;
    }
    ...
    $adminNoti->addNoti($request_noti);
    return true;
}


function notificationCusMutil($type = 'remind', $list_client = [], $dataAppend = [])
{
    $adminNoti = new \App\Services\NotiService();
    $dtObject = [];
    $title_point = '';
    ...
    $dtClient = Clients::whereIn('id', $list_client)->get();
    $arr_object_id = [];
    $dtCustomer = Clients::select(
        'tbl_clients.fullname as name',
        'tbl_clients.id as object_id',
        'tbl_clients.lang_default as lang',
        'tbl_player_id.player_id as player_id',
        DB::raw("'customer' as 'object_type'")
    )
        ->leftJoin('tbl_player_id', function ($join) {
        ...
        ->whereIn('tbl_clients.id', $list_client)
        ->get()->toArray();
    if (!empty($dtCustomer)) {
        $arr_object_id = array_merge($arr_object_id, $dtCustomer);
    }
    ...
    $infoClient = [];
    foreach ($dtClient as $key => $client) {
        $infoClient[] = [
            'id' => $client->id,
            'locale' => $client->lang_default ?? Config::get('constant')['lang_default'],
        ];
    }
    $request_noti = new Request([
        'type_noti' => $type,
    ...
    $adminNoti->addNotiMutil($request_noti);
    return true;
}


//kiểm tra chiến dịch hoàn thành và gửi thông báo
function challenge_success($id_challenge_me)
{
    $challengeMe = ChallengeMe::select('tbl_challenge_me.*', 'tbl_clients.lang_default')
        ->where('tbl_challenge_me.status', '=', 1)
        ->where('tbl_challenge_me.id', '=', $id_challenge_me)
    ...
    foreach ($listLang['data'] as $key => $value) {
        ...
        $dataNotify[$value['code']]['content'] = lang('notify_challenge_success', 'message', [
            'name' => $challengeStran->name
        ], $value['code']);

        $dataNotify[$value['code']]['title'] = lang('title_notify_challenge_success', 'message', [
            'name' => $challengeStran->name
        ], $value['code']);
    }
    $dataNotify['id_challenge'] = $challengeMe->id_challenge;
    $dataNotify['id_challenge_me'] = $challengeMe->id;
    notificationCus('challenge_me_success', 0, $challengeMe->client_id, $dataNotify);
    $challengeMe->status = 1;
    if (empty($challengeMe->plus_refund)) {
        $dtObject = changePoint($challengeMe->id, 'plus_refund');// hoàn thành thử thách được hoàn điểm
        if (!empty($dtObject)) {
            $challengeMe->plus_refund = 1;
        }
    }
    $challengeMe->save();
    return true;
...
function extractAndNormalizeChallengeCode($content)
{
    // Chuẩn hóa content
    ...
    if (preg_match('/^CMW[\s\-]?(\d{10})\b/i', $content, $matches)) {
        return 'CMW-' . $matches[1];
    }

    return null;
}

function extractAndNormalizePaymentCode($content)
{
    $content = preg_replace('/[^A-Za-z0-9]/', ' ', $content);
    $content = preg_replace('/\s+/', ' ', $content);
    $content = trim($content);
    if (preg_match('/\b([A-Z]{2})[\s-]?(\d{9})\b/', $content, $matches)) {
        return $matches[1] . '-' . $matches[2];
    } elseif (preg_match('/\b([A-Z]{1})[\s-](T)?(\d{9})\b/', $content, $matches)) {
        return 'PT-' . $matches[3];
    }
    return null;
}

function getListTypePartner($id = 0, $type = 'name')
{
    $data = [
        [
            'id' => 1,
...
function get_parent_customer_leader($customer_id)
{
    return get_parent_customer_leader_id_helper($customer_id);
}

function get_parent_customer_leader_id_helper($child_id = 0, $depth = 0)
{
    if ($depth > 30) {
        return 0;
    }

    $item = \App\Models\ClientIntroduce::with('parent')
        ->where('id_client', $child_id)
        ->first();
    ...
    if ($item->parent->is_leader == 1) {
        return $item->parent->id;
    }

    return get_parent_customer_leader_id_helper($item->parent->id, $depth + 1);
}

function getBranchChild($ancestor, $descendant)
{
    $map = \App\Models\ClientIntroduce::pluck('id_client_introduce','id_client');

    $current = $descendant;

    while ($current > 0) {

        $parent = $map[$current] ?? 0;
