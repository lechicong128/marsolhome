...
trait UploadFile
{
    public function UploadFile_bk($file = array(), $folder = null,$with = 150,$height = 150,$resize = true,$disk = 'public')
    {
    ...
    public function UploadFile($file = array(), $folder = null, $with = 150, $height = 150, $resize = true, $disk = 'public')
    {
        ...
        if (is_array($file)) {
            ...
            foreach ($file as $key => $value) {
                ...
                $filetype = strtolower($value->getClientOriginalExtension());
                $fileName = $disk . '/' . $folder . '/' . $file_name;

                if (in_array($filetype, $arr)) {

                    // ✅ FIX: GIF phải lưu nguyên bản, nếu đi qua Img::make()->resize()->save() sẽ mất animation (chỉ còn frame đầu)
                    if ($filetype === 'gif') {
                        if (!Storage::disk($disk)->exists($folder)) {
                            Storage::disk($disk)->makeDirectory($folder);
                        }
                        $path = $value->storeAs($folder, $file_name, $disk); // trả về: $folder/$file_name
                        array_push($arrPath, $path);
                        continue;
                    }
                    ...
                    $img = Img::make($value)->orientate();

                    if (!empty($resize)) {
                        $path = $img->resize($with, $height, function ($const) {
                            $const->aspectRatio();
                        })->save(storage_path('app/' . $fileName));
                    } else {
                        $path = $img->save(storage_path('app/' . $fileName));
                    }
            ...
            if (in_array($filetype, $arr)) {
                ...
                // ✅ FIX: GIF lưu nguyên bản để giữ chuyển động
                if ($filetype === 'gif') {
                    if (!Storage::disk($disk)->exists($folder)) {
                        Storage::disk($disk)->makeDirectory($folder);
                    }
