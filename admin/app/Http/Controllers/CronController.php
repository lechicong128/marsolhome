...
//    public function sendNotiRemindReview() {
//        $day = get_option('number_day_remind_review') ?? 0;
//        $date = new DateTime();
//        $date->modify("+$day days");
//        $date = $date->format('Y-m-d H:i:s');
//        $SignUpNotReview = DB::table('tbl_sign_up_review')
//            ->whereIn('status', [4, 5])
//            ->where('date_success', '<=', $date)
//            ->whereRaw("NOT EXISTS (SELECT 1 FROM tbl_clients_sign_up_review WHERE id_review = tbl_sign_up_review.id AND is_review = 1)")
//            ->where('send_noti_remind_review', 0)
//            ->limit(10)
//            ->get();
//        $arrayInfoSendZalo = [];
//        $listIDClient = [];
//        if(!empty($SignUpNotReview)) {
//            foreach($SignUpNotReview as $key => $value) {
//                Notification::notiRemindReview($value->id, 'remind_review');
//                $arrayInfoSendZalo[] = [
//                    'date_receive' => date("H:i d/m/Y", strtotime($value->date_success)),
//                    'order_code' => $value->code_review,
//                    'id_client' => $value->id_client,
//                    'status_review' => status_product_review($value->status)['name'] ?? '',
//                ];
//                $listIDClient[] = $value->id_client;
//            }
//        }
//
//        $this->request->merge(['list_id' => array_values(array_unique($listIDClient))]);
//        $responseClient = $this->dbAccount->getListDetailCustomer($this->request);
//        $dataClient = $responseClient->getData(true);
//        $client = $dataClient['clients'] ?? [];
//        if(!empty($client)) {
//            foreach ($arrayInfoSendZalo as $key => $value) {
//                $is_client = $client[$value['id_client']] ?? [];
//                if (empty($is_client)) {
//                    continue;
//                }
//                $dataSendZalo = [
...
//                    'customer_name' => $is_client['fullname'] ?? '',
//                    'status_review' => $value['status_review'],
//                ];
//                send_zalo($is_client['phone'], 'changeStatus', 493204, $dataSendZalo);
//            }
//        }
//
//
//        $SignUpReview = DB::table('tbl_sign_up_review')
//            ->whereIn('status', [4, 5])
//            ->where('date_success', '<=', $date)
//            ->whereRaw("EXISTS (SELECT 1 FROM tbl_clients_sign_up_review WHERE id_review = tbl_sign_up_review.id AND is_review = 1)")
//            ->where('send_noti_remind_review', 0)
//            ->limit(10)
//            ->get();
//        if(!empty($SignUpReview)) {
//            foreach($SignUpReview as $key => $value) {
//                DB::table('tbl_sign_up_review')
//                    ->where('id', $value->id)
//                    ->update(['send_noti_remind_review' => 1]);
//            }
//        }
//
//    }

//    public function render_video_review() {
//        $reviewRendering = ClientsReview::where('is_render_view', 1)
//            ->where('is_review', 1)
//            ->orderBy('date_review', 'asc')->get();
//
//        if(!empty($reviewRendering)) {
//            foreach($reviewRendering as $key => $value) {
//                if(!empty($value->video_review_render) && file_exists(storage_path('app/public/' . $value->video_review_render))) {
//                    $value->is_render_view = 2;
//                    $value->video_review_render = $value->video_review_render;
//                    $value->save();
//                }
//            }
//        }
//
//        $review = ClientsReview::where('is_render_view', 0)
//            ->where('is_review', 1)
//            ->orderBy('date_review', 'asc')->first();
//        if(!empty($review)) {
//            if(!empty($review->video_review) && file_exists(storage_path('app/public/' . $review->video_review))) {
//                $input = storage_path('app/public/' . $review->video_review);
//                $output = storage_path('app/public/video_size/' . $review->video_review);
//                $review->is_render_view = 1;
//                $review->video_review_render = 'video_size/' . $review->video_review;
//                $review->save();
//                $this->compressVideo($input, $output, 28);
//            }
//            else {
//                $review->is_render_view = 3;
//                $review->save();
//            }
//        }
//    }
//    public function render_image_video_review() {
//        $review = ClientsReview::whereRaw('video_review IS NOT NULL')
//            ->where('is_review', 1)
//            ->whereRaw('small_image_video_review IS NULL')
//            ->orderBy('date_review', 'asc')
//            ->first();
//        if(!empty($review)) {
//            if(!empty($review->video_review) && file_exists(storage_path('app/public/' . $review->video_review))) {
//                $input = storage_path('app/public/' . $review->video_review);
//                $output = storage_path('app/public/image_video_size/' . $review->id.'.png');
//                $review->small_image_video_review = 'image_video_size/' . $review->id.'.png';
//                $review->save();
//                $this->ImagecompressVideo($input, $output);
//            }
//        }
//    }
    public function render_video_file() {
        $video = VideoFile::whereRaw('video IS NULL')
            ->whereRaw('original_video IS NOT NULL')
            ->where('render_video', 0)
        ...
        if(!empty($video)) {
            ...
            if(file_exists($input)) {
                ...
                $link_video = 'video_size/' . $video->original_video;
                $this->compressVideo($input, $output, 28);
                if (file_exists($output)) {
                    $data_video = VideoFile::find($video->id);
                    $data_video->video = $link_video;
                    $data_video->save();
                }
                echo $link_video;
            }
            $video->render_video = 1;
            $video->save();
        }
    }
    public function render_thumbnail_file() {
        $video = VideoFile::whereRaw('thumbnail IS NULL')
            ->whereRaw('video IS NOT NULL')
            ->orderBy('created_at', 'asc')
            ->first();
        if(!empty($video)) {
            $input = storage_path('app/public/' . $video->video);
            if(file_exists($input)) {
                $output = storage_path('app/public/image_thumbnail/' . $video->id.'.png');
                $link_img = 'image_thumbnail/' . $video->id.'.png';
                $this->ImagecompressVideo($input, $output, 28);
                if (file_exists($output)) {
                    $data_video = VideoFile::find($video->id);
                    $data_video->thumbnail = $link_img;
                    $data_video->save();
                }
                echo $link_img;
            }
    ...
    function SendMailCreateOrders($id = '') {
        Sendmail::SendMailCreateOrders($id);
    }
    public function cronjob_send_mail() {
        $dtCronEmail = DB::table('tbl_cron_email')->where('status', 0)->orderBy('id', 'asc')->limit(10)->get();
        if(!empty($dtCronEmail)) {
            foreach ($dtCronEmail as $key => $value) {
                if($value->type == 1) { // 1 là loại email gửi cho đơn hàng mới
                    Sendmail::SendMailCreateOrdersNew($value->id_ref); 
                } elseif($value->type == 2) { // 2 là email gửi cho lịch hẹn spa
                    Sendmail::SendMailCreateBookingNew($value->id_ref);
                }
                DB::table('tbl_cron_email')->where('id', $value->id)->update([
                    'status' => 1
                ]);
            }
        }
