...
class SettingsController extends Controller
{
    ...
    public function get_list(){
        ...
        if($group == 'feedback_improve') {
            $response = $this->ServiceAccount->GetFeedbackImprove($this->request);
            $dataRes = $response->getData(true);
            $dataMain = $dataRes['data'];
            $data = [];
            foreach($dataMain as $key => $value) {
                $dataStran = [];
                foreach($value['translations'] as $trans) {
                    $dataStran[$trans['language']] = $trans['name'];
                }
                $data[$value['id_star_like']][] = [
                    'key_main' => $value['key_main'],
                    'translations' => $dataStran
                ];
            }
        }
        if($group == 'terms') {
    ...
    public function submit($group = 'info') {
        ...
        if($group == 'feedback_improve') {
            $response = $this->ServiceAccount->updateFeedbackImprove($this->request);
            $dataRes = $response->getData(true);
            $data = $dataRes['data'];
            if($data['result'] == false) {
                return redirect('admin/settings?group=' . $group)->with('error', $data['message']);
            }
            else {
                $app = new App();
                $app->flushCache();
            ...
            if (!empty($listData) || !empty($listDataFiles)) {
                if (!empty($listData)) {
                    ...
                    foreach ($listData as $key => $value) {
                        ...
                        if ($group == 'haru_wallet'){
                            // if($value)
                            if($key == 'exchange_rate_haru_wallet'){

                                if(((string)$value) != get_option('exchange_rate_haru_wallet')){
                                    $staff_id = get_staff_user_id();
                                    //lưu lịch sử thay đổi tỉ giá haru wallet
                                    DB::table('tbl_history_change_haru')->insert([
                                        'exchange_rate_old' => get_option('exchange_rate_haru_wallet'),
                                        'exchange_rate_new' => $value,
                        ...
                        if ($group == 'referral_program' || $group == 'banner_event' || $group == 'review_haru_wallet') {
                            $arrKey = ['percent_person_referral','percent_referral','money_max_referral','number_coins_received_review'];
