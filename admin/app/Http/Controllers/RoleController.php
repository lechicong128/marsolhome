        ...
        return Datatables::of($role)
            ...
            ->addColumn('permission', function ($role) {
                ...
                if ($role->permission->count() > 0) {
                    ...
                    $grouped = $role->permission->groupBy(function ($item) {
                        return $item->groupPermission->id;
                    });
            
                    foreach ($grouped as $group) {
                        $groupName = $group->first()->groupPermission->name;
            
                        $str .= "<div>$groupName</div>";
            
                        foreach ($group as $value) {
                            $str .= "<div class='label label-success'>" . lang($value->name) . "</div> ";
                        }
                    }
                }
