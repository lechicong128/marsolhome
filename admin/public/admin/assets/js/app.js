...
function InitDataTableNew(selector, url, initParams, notsearchable = [0], notsortable = [0], fnserverparams = [], defaultorder = [0], fixedColumns = { leftColumns: 0, rightColumns: 0 }) {
    ...
    // THỰC THI PHÒNG THỦ: Luôn gán cứng width: 100% cho table để tránh lỗi lệch cột hệ thống
    table.css('width', '100%');
                ...
                .done(function (data) {
                    if (data) {
                        if (data.result) {
                            ...
                            if (typeof oTableTransaction != 'undefined') {
...
function searchAjaxSelect2Img(element, url = '', id = 0, paramsCus = {}, allowClear = true, opts = {}) {
    const $el = $(element);

    // 1) Lấy data-json ban đầu
    let items = [];
    ...
    $el.select2({
        ...
        ajax: url ? {
            ...
            dataType: 'json',
            delay: 250,
            data: params => ({ paramsCus, term: params.term }),
            processResults: data => {
                const list = (data.items || data).map(it => ({
                    id: String(it.id ?? it.value),
                    text: it.text ?? it.full_name ?? it.name ?? '',
                    image: it.image || it.avatar || it.photo || null,
                    ...it
                }));
                return { results: list };
            },
...
// function repoFormatSelectionPrice(state) {
//     if (!state.id) return state.text;
//     if(!state.img_thumbnail) {
//         if (state.img) {
...
//     else {
//         if (state.img_thumbnail) {
//             var img = '<img class="img_option" src="' + site_url + state.img_thumbnail + '"/> ';
//         }
//         else {
//             var img = '<img class="img_option" src="' + site_url + 'download/preview_image"/> ';
//         }
//     }
//     var text = state.text;
//     if(state.style_color) {
//         text = '<p style="color: '+state.style_color+'">' + state.text + '</p>';
//     }
//
//     var Stringreturn = img + text;
//     if (state.price) {
//         Stringreturn += ' - ' + C_formatNumber(Math.round(state.price / 1000)) + 'K'
//     }
//
//     return Stringreturn;
// }
