...
<div class="row">
    <div class="col-sm-12">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="admin/invoice/list">{{$title}}</a></li>
...
<div class="row">
    <div class="col-sm-12">
        <div class="card-box table-responsive">
            ...
            <table id="table_invoice" class="table table-bordered table_invoice">
                <thead>
                <tr>
                    <th class="text-center">{{lang('dt_stt')}}</th>
                    <th class="text-center">{{lang('Ngày tạo đơn hàng')}}</th>
                    <th class="text-center">{{lang('Số hóa đơn hệ thống')}}</th>
                    <th class="text-center">{{lang('Số hóa đơn thuế')}}</th>
                    ...
                    <th class="text-center">{{lang('Trạng thái nháp')}}</th>
...
@section('script')
    <script>
        ...
        var oTable;
        oTable = InitDataTable('#table_invoice', 'admin/invoice/getListInvoice', {
            'order': [
                [0, 'desc']
            ],
            'responsive': true,
            "ajax": {
                "type": "POST",
                "url": "admin/invoice/getListInvoice",
                "data": function (d) {
            ...
            columnDefs: [
                ...
                {data: 'reference_no_bill', name: 'reference_no_bill',width: "200px" },
                {data: 'reference_no_transaction', name: 'reference_no_transaction',width: "200px" },
                ...
                {
                    data: 'options', name: 'options', width: "110px",
                    "render": function(data, type, row) {
                        return `<div class="text-center">${data}</div>`;
                    }
                }
        ...
        $('#table_invoice').on('draw.dt', function () {
            ...
            var total_vat =  table.column(7).data().sum();
            var grand_total =  table.column(8).data().sum();
        
            $("#table_invoice").find('tfoot .total').html(formatNumber(total));
            $("#table_invoice").find('tfoot .total_vat').html(formatNumber(total_vat));
            $("#table_invoice").find('tfoot .grand_total').html(formatNumber(grand_total));
        });
