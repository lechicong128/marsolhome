<form id="termsForm" action="admin/terms/submit/{{$id}}" method="post" data-parsley-validate novalidate>
    {{csrf_field()}}
    <style>
        .tab-btn {
            flex: 1;
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        <input type="hidden" name="id" value="{{!empty($terms) ? $terms->id : 0}}" >
                        ...
                        <input type="hidden" name="type" class="form-control" value="{{$terms->type ?? 1}}"/>
                                ...
                                <div id="tab-info-{{$lang->code}}" class="tab-pane fade {{$lang->is_default ? 'in active' : ''}}">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="title[{{$lang->code}}]"
                                                  id="title_{{$lang->code}}"
                                                  class="form-control" value="{{$terms->translations[$lang->code]['title'] ?? ''}}"/>
                                    </div>
                                    <div class="form-group">
                                        <label for="content_{{$lang->code}}">{{lang('content_terms'.$types_terms)}}</label>
                                        <textarea type="text" name="content[{{$lang->code}}]"
                                                   id="content_{{$lang->code}}"
                                                   class="form-control editor_short">{{$terms->translations[$lang->code]['content'] ?? ''}}</textarea>
                                    </div>
                                </div>
                            @endforeach
                    </div>
