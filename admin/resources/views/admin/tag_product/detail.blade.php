<form id="tagProductForm" action="admin/tag_product/submit/{{$id}}" method="post" data-parsley-validate novalidate>
    {{csrf_field()}}
    <style>
        .tab-btn {
            flex: 1;
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <input type="hidden" name="type" class="form-control" value="{{$dtData->type ?? 1}}"/>
                        <div class="tab-content row" style="padding-left:10px;padding-right:10px;">
                            <div class="form-group">
                                <label for="color">{{lang('dt_color')}}</label>
                            ...
                            @foreach($language as $lang)
                                <div id="tab-info-{{$lang->code}}" class="tab-pane fade {{$lang->is_default ? 'in active' : ''}}">
                                    <div class="form-group">
                                        <label for="name{{$lang->code}}">{{lang('dt_tag_product_name')}}</label>
                                        <input required type="text" name="name[{{$lang->code}}]"
                                               id="name_{{$lang->code}}"
                                               class="form-control" value="{{$dtData->translations[$lang->code]['name'] ?? ''}}"/>
                                    </div>
                                </div>
                            @endforeach
                        </div>
