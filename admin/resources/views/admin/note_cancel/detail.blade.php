<style>
    ...
    .nav.nav-justified > li > a, .nav.tabs-vertical > li > a{
        line-height: 25px;
    }
</style>
<form id="noteCancelForm" action="admin/note_cancel/submit/{{$id}}" method="post" data-parsley-validate novalidate>
    {{csrf_field()}}
                            ...
                            @foreach($language as $lang)
                                <div id="tab-info-{{$lang->code}}" class="tab-pane fade {{$lang->is_default ? 'in active' : ''}}">
                                    <div class="form-group">
                                        <label for="note{{$lang->code}}">{{lang('dt_note')}}</label>
                                        <input required type="text" name="note[{{$lang->code}}]" class="form-control" id="note{{$lang->code}}"  value="{{$noteCancel->translations[$lang->code]['note'] ?? ''}}">
                                    </div>
                                </div>
                            @endforeach
                        </div>
