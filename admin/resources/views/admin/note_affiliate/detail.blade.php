...
<form id="PaymentModeForm" action="admin/note_affiliate/submit/{{$id}}" method="post" data-parsley-validate novalidate>
    {{csrf_field()}}
                    ...
                    <div class="col-md-12">
                        ...
                        @foreach($language as $lang)
                            <div id="tab-info-{{$lang->code}}" class="tab-pane fade {{$lang->is_default ? 'in active' : ''}}">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="title[{{$lang->code}}]" class="form-control title_{{$lang->code}}" value="{{$dtData->translations[$lang->code]['title'] ?? ''}}">
                                </div>
                                <div class="form-group">
                                    <label for="content{{$lang->code}}">{{lang('c_content')}}</label>
                                    <textarea name="content[{{$lang->code}}]" class="form-control content_{{$lang->code}}">{{$dtData->translations[$lang->code]['content'] ?? ''}}</textarea>
                                </div>
                            </div>
                        @endforeach
                        <div class="form-group hide">
                            ...
                            <select class="type select2 form-control" name="type" id="type">
                                <option value="1" {{!empty($dtData) && $dtData['type'] == 1 ? 'selected' : ''}}>{{lang('dt_note_review')}}</option>
                                <option value="2" {{!empty($dtData) && $dtData['type'] == 2 ? 'selected' : ''}}>{{lang('dt_review')}}</option>
                            </select>
                        </div>
                        </div>
                    </div>
