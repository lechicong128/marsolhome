    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="btn-group pull-right m-t-15">
                <a type="button" class="btn btn-default dropdown-toggle waves-effect waves-light dt-modal" href="admin/products_filter/detail/0">{{ lang('dt_create') }}</a>
            </div>
            <h4 class="page-title text-capitalize">{{ lang('products_filter') }}</h4>
            <ol class="breadcrumb">
                <li><a href="admin/dashboard">{{ lang('dt_index') }}</a></li>
                <li><a href="admin/products_filter/list">{{ lang('products_filter') }}</a></li>
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="card-box table-responsive">
                ...
                <hr/>
                <table id="table_products_filter" class="table table-bordered table-condensed sortableMain table_render">
                    <thead>
                    <tr>
                        <th class="text-center">{{ lang('dt_stt') }}</th>
                        <th class="text-center">{{ lang('c_group_products_filter') }}</th>
                        <th class="text-center">{{ lang('c_active') }}</th>
                        <th class="text-center">{{ lang('filter_main_app') }}</th>
...
@section('script')
    <script>
        ...
        function changeType(_this, _id, _status) {
            ...
            $.ajax({
                type: "GET",
                url: 'admin/products_filter/changeType',
                data: dataGET,
                dataType: "json",
        ...
        oTable = InitDataTable('#table_products_filter', 'admin/products_filter/getTable', {
            'order': [
                [0, 'asc']
            ],
            'responsive': true,
            "ajax": {
                "type": "POST",
                "url": "admin/products_filter/getTable",
                "data": function(d) {
            ...
            columnDefs: [{
                ...
                },
                {
                    ...
                    data: 'filter_main_app',
                    name: 'filter_main_app',
                    width: '80px'
                },
                {
                    data: 'options',
        ...
        var listTr = {};
        var activeFirst = 0;
        $('#table_products_filter').on('draw.dt', function () {
            if(activeFirst == 0) {
        ...
        function loadInfoOrder(cData) {
            ...
            thead.append(`<tr class="not-sortable">
                                <th>{{lang('c_stt')}}</th>
                                <th>{{lang('c_name_products_filter')}}</th>
                                <th class="text-center">{{ lang('c_active') }}</th>
                                <th class="text-center">{{ lang('filter_main_app') }}</th>
            ...
            $.each(dataChild, function(index, value) {
                tbody.append(`<tr class="not-sortable">
                            ...
                            <td>${value.name}</td>
                            <td>${value.active}</td>
                            <td>${value.filter_main_app}</td>
                            <td class="text-right">
                                <a class="btn btn-icon btn-default dt-modal" href="admin/products_filter/detail_child/${value.id_parent}/${value.id}"><i class="fa fa-edit"></i></a>
                                <a type="button" class="btn btn-danger btn-icon po-delete" data-container="body" data-html="true"
                                    data-toggle="popover" data-placement="left"
                                    data-content="<button href='admin/products_filter/delete/${value.id}'
                                    class='btn btn-danger dt-delete' style='margin-right: 5px;'>Xóa</button><button class='btn btn-default po-close'>Đóng</button>" data-original-title="" title=""><i class="fa fa-remove"></i>
                                </a>
                            </td>
                        </tr>`);
            })
        ...
        function EventUpdateSorMain(_this) {
            if (confirm('Bạn có chắc muốn sắp xếp danh mục?')) {
                ...
                var limit = $('[name="table_products_filter_length"]').val();
                ...
                $.ajax({
                    type: "POST",
                    url: 'admin/products_filter/order_by',
                    data: data,
                    dataType: "json",
                    success: function(response) {
                        if (response.result) {
