    ...
    <div class="row">
        <div class="col-sm-12">
            ...
            <ol class="breadcrumb">
                ...
                <li class="active">{{lang('c_feedback')}}</li>
            </ol>
        </div>
    </div>
    <div class="row">
        <form id="feedbackForm" action="admin/admin_website/submit_feedback" method="post" data-parsley-validate
              novalidate
                                ...
                                @if(!empty($lang_current))
                                    ...
                                    <div class="div-lang div-{{$value['code']}}">
                                        <div class="form-group">
                                            <label for="title_section section_{{$value['code']}}_title_main">{{lang('c_title_main')}} - {{$value['name']}}</label>
                                            <input type="text" name="section[{{$value['code']}}][title_main]"
                                                      id="section_{{$value['code']}}_title_main"
                                                      class="form-control" value="{{$feedback[$value['code']]->title_main ?? ''}}">
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <textarea type="text" name="section[{{$value['code']}}][title]"
                                                      ...
                                                      class="form-control editor_short">{{$feedback[$value['code']]->title ?? ''}}</textarea>
                                        </div>
                                        <div class="form-group">
                                            <label for="section{{$value['code']}}_content">{{lang('c_content')}}</label>
                                            <textarea type="text" name="section[{{$value['code']}}][content]"
                                                      id="section_{{$value['code']}}_content"
                                                      class="form-control">{{$feedback[$value['code']]->content ?? ''}}</textarea>
                                        </div>
                                    </div>
                                    @endforeach
                                    <div class="form-group">
                                        <label for="image">{{lang('images')}}</label><br>
                                        <input name="image" type="file" id="image" style="display: none;" accept="image/*">
                                        <input name="image_before" type="hidden" value="{{$feedback[$lang_default]->image ?? ''}}">
                                        <img data-id="image" class="preview-image"
                                             onerror="this.onerror=null; this.src='admin/assets/images/not_available.jpg';"
                                             src="{{asset('storage/' . ($feedback[$lang_default]->image ?? ''))}}" title="{{lang('c_click_in_change_image')}}">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    @if(!empty($lang_current))
                                        ...
                                        <div class="div-lang div-{{$value['code']}}">
                                            <div class="form-group">
                                                <label for="title_section section_{{$value['code']}}_title_experience">{{lang('c_title_experience')}} - {{$value['name']}}</label>
                                                <input type="text" name="section[{{$value['code']}}][title_experience]"
                                                          id="section_{{$value['code']}}_title_experience"
                                                          class="form-control" value="{{$feedback[$value['code']]->title_experience ?? ''}}"/>
                                            </div>
                                            <hr/>
                                            <div class="form-group">
                                                <label for="title_section section_{{$value['code']}}_title_improve">{{lang('c_title_improve')}} - {{$value['name']}}</label>
                                                <input type="text" name="section[{{$value['code']}}][title_improve]"
                                                          id="section_{{$value['code']}}_title_improve"
                                                          class="form-control" value="{{$feedback[$value['code']]->title_improve ?? ''}}"/>
                                            </div>
                                            <hr/>
                                            <div class="form-group">
                                                <label for="title_section section_{{$value['code']}}_title_feedback">{{lang('c_title_feedback')}} - {{$value['name']}}</label>
                                                <input type="text" name="section[{{$value['code']}}][title_feedback]"
                                                          id="section_{{$value['code']}}_title_feedback"
                                                          class="form-control" value="{{$feedback[$value['code']]->title_feedback ?? ''}}"/>
                                            </div>
                                        </div>
                                        @endforeach
                                    @endif
