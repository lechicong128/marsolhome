...
//                ->whereHas('transfer_money')
    ...
    if (!empty($dtObject)) {
        ...
        if(!in_array($type,['reward_profit','reward_ranking_bonus','adjust_balance'])) {
            ...
            if (!empty($revenue)) {
                Notification::notiChangeBalanceOwner($customer_id, $revenue, $account_balance_client, $title_balance,
                    $type_driver);
            }
            if ($type == "transfer_money") {
                ...
                if (!empty($revenue_receive)) {
                    Notification::notiChangeBalanceOwner($customer_id_receive, $revenue_receive, $account_balance_client_receive,
                        $title_balance, $type_driver);
                }
            }
        }
        return true;
...
function changePoint($id = 0, $type = '')
{
    ...
    if ($type == 'finish') {
        $dtObject = Transaction::select(
            ...
            ->where('status', Config::get('constant')['status_transaction_finish'])
            ->where('tbl_transaction.id', $id)
            ->first();
        ...
        $dtObject = TransactionDriver::select(
            ...
            ->where('status', Config::get('constant')['status_finish_driver'])
            ->where('tbl_transaction_driver.id', $id)
            ->first();
...
if (!function_exists('lang')) {
    function lang($message = null, $key = 'message')
    {
        ...
        $fullLangLater = trans($key . '.' . $message);
